...
def generate_registration_code(length: int = 12) -> str:
    ...
    """
    # Используем буквы и цифры, исключая похожие символы: 'O' (буква O), '0' (ноль), 'I' (заглавная i), '1' (единица), 'L' (заглавная L)
    # Это делается для повышения читаемости и чтобы избежать путаницы между похожими символами.
